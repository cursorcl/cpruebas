...
public class AlumnosTableTreeView extends AFormView implements
	...
	private void accionClicTabla() {
		tblAlumnos.setOnMouseClicked(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent event) {
				ObservableList<TreeItem<OTAlumno>> selected = tblAlumnos
	...
	private void accionEliminar() {
		ObservableList<TreeItem<OTAlumno>> otSeleccionados = tblAlumnos
				.getSelectionModel().getSelectedItems();
	...
	private void inicializaTabla() {
		...
		colId.setCellValueFactory(new TreeItemPropertyValueFactory<OTAlumno, Long>(
				"id"));
		...
		for (TreeItem<OTAlumno> item : root.getChildren()) {
			...
			if (ot.getColegio().equals(otAlumno.getColegio())) {
				for (TreeItem<OTAlumno> itemCurso : item.getChildren()) {
					if (itemCurso.getValue().getCurso()
							.equals(otAlumno.getCurso())) {
						for (TreeItem<OTAlumno> itemAlumno : itemCurso
								.getChildren()) {
		...
		if (list != null && !list.isEmpty()) {
			...
			if (entity instanceof Alumno) {
				...
				for (Object iEntity : list) {
					...
					if (itemColegio == null
							|| !ot.getColegio().equals(
									itemColegio.getValue().getColegio())) {
