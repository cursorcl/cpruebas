...
public class DefinePruebaViewController extends AFormView {
    ...
    @FXML private TableView<RegistroDefinePrueba> tblRegistroDefinePrueba;
    @FXML private TableColumn<RegistroDefinePrueba, Integer> preguntaCol;
    @FXML private TableColumn<RegistroDefinePrueba, String> respuestaCol;
    @FXML private TableColumn<RegistroDefinePrueba, Boolean> vfCol;
    @FXML private TableColumn<RegistroDefinePrueba, Boolean> mentalCol;
    @FXML private TableColumn<RegistroDefinePrueba, R_Habilidad> habCol;
    @FXML private TableColumn<RegistroDefinePrueba, R_Ejetematico> ejeCol;
    @FXML private TableColumn<RegistroDefinePrueba, R_Objetivo> objCol;
    @FXML private TableColumn<RegistroDefinePrueba, String> badCol;
    @FXML private TableView<R_Ejetematico> tblEjesTematicos;
    @FXML private TableColumn<R_Ejetematico, String> ejeTematicoCol;
    @FXML private TableView<R_Objetivo> tblObjetivos;
    @FXML private TableColumn<R_Objetivo, String> objetivoCol;
    @FXML private TableView<R_Habilidad> tblHabilidades;
    @FXML private TableColumn<R_Habilidad, String> habilidadCol;
    @FXML private MenuItem mnuGrabar;
    @FXML private MenuItem mnuExportar;
    @FXML private Button btnListo;
    @FXML private TextField txtRespuestas;
    ...
    private List<R_RespuestasEsperadasPrueba> getRespuestasEsperadas() {
        ...
        List<R_RespuestasEsperadasPrueba> fromPrueba = controller.findByParamsSynchro(R_RespuestasEsperadasPrueba.class, params);
        if (fromPrueba == null) {
        ...
        if (fromPrueba.size() < registros.size()) {
            while (n < registros.size()) {
                final RegistroDefinePrueba registro = registros.get(n);
                final R_RespuestasEsperadasPrueba respuesta = new R_RespuestasEsperadasPrueba.Builder().id(Utils.getLastIndex()).build();
                respuesta.setEjetematico_id(registro.getEjeTematico().getId());
                respuesta.setHabilidad_id(registro.getHabilidad().getId());
                respuesta.setMental(registro.getMental());
                respuesta.setName(registro.getNumero().toString());
    ...
    public void initialize() {
        ...
        tblEjesTematicos.setOnDragDetected(event -> {
            final Dragboard db = tblHabilidades.startDragAndDrop(TransferMode.ANY);
            final ClipboardContent content = new ClipboardContent();
            content.putIfAbsent(EjeTematicoDND.ejeTematicoTrackDataFormat, tblEjesTematicos.getSelectionModel().getSelectedItem());
            db.setContent(content);
            event.consume();
        });
        ...
        if (entity instanceof R_Prueba) {
            ...
            if (respuestas != null && !respuestas.isEmpty()) {
                ...
                for (final R_RespuestasEsperadasPrueba respuesta : respuestas) {
                    ...
                    if (eje == null) eje = controller.findByIdSynchro(R_Ejetematico.class, respuesta.getEjetematico_id());
                    R_Habilidad habilidad = habilidades.get(respuesta.getHabilidad_id());
                    if (habilidad == null) habilidad = controller.findByIdSynchro(R_Habilidad.class, respuesta.getHabilidad_id());
                    R_Objetivo objetivo = objetivos.get(respuesta.getObjetivo_id());
                    if (objetivo == null) objetivo = controller.findByIdSynchro(R_Objetivo.class, respuesta.getObjetivo_id());
            ...
            final boolean editable = !Estado.getEstado(prueba.getEstado()).equals(Estado.EVALUADA);
    ...
    private void registerDND() {
        tblRegistroDefinePrueba.setOnDragDropped(dragEvent -> {
            final ObservableList<RegistroDefinePrueba> seleccionados = tblRegistroDefinePrueba.getSelectionModel().getSelectedItems();
            if (seleccionados != null && !seleccionados.isEmpty()) {
                if (dragEvent.getDragboard().getContent(EjeTematicoDND.ejeTematicoTrackDataFormat) != null) {
                    final R_Ejetematico ejeTematico = (R_Ejetematico) dragEvent.getDragboard().getContent(EjeTematicoDND.ejeTematicoTrackDataFormat);
                    ...
                    final R_Habilidad habilidad = (R_Habilidad) dragEvent.getDragboard().getContent(HabilidadDND.habilidadTrackDataFormat);
                    ...
                    final R_Objetivo objetivo = (R_Objetivo) dragEvent.getDragboard().getContent(ObjetivoDND.objetivoTrackDataFormat);
