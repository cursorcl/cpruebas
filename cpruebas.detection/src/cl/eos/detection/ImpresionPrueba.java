...
public class ImpresionPrueba {
	...
	private static final int[] RUT_COLS = { 62, 77, 95, 110, 125, 145, 160,
			175, 194 };
	private static final int[] RUT_ROWS = { 84, 97, 110, 123, 136, 149, 159,
			172, 185, 198, 211, 224};
	...
	public void imprimir(Prueba prueba, Curso curso, Profesor profesor,
		...
		try {
			...
			File output = new File("./res/" + prueba.getName() + ".pdf");
			doc.save(output);
			doc.close();
	...
	private void drawRut(String rut) {
		...
		for (char ch : chrRut) {
			...
			g2.fillOval(RUT_COLS[idxCol], RUT_ROWS[idxRow], CIRCLE_WIDTH,
					CIRCLE_WIDTH);
			idxCol++;
		}
	}
