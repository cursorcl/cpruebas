...
public abstract class AExtractorResultados implements IExtractorResultados {
  ...
  /**
   * Comparador creado especificamente para ordenar los pares de valores que se obtienen al procesar
   * el RUT. Ordena por el primer item que en este caso representa el valor menor de la corrdenada
   * X.
   * 
   * @return El valor correspondiente indicando si es menor, igual o mayor el primero par con
   *         respecto al segundo.
   */
  ...
  protected String getRut(Point pRefRut, BufferedImage image) {
    ...
    for (int n = 0; n < sortedPairs.size(); n++) {
      ...
      int width = minMax.getSecond().intValue() - minMax.getFirst().intValue();
      BufferedImage rut = sectorRut.getSubimage(start, 0, width, 545);

      writeIMG(rut, "RUT_" + (nRut));
      nRut++;
      Pair<Integer, Pair<Double, Double>> result = recognizerRut.recognize(rut, 0.40);
  ...
  protected final Point getPointReferenciaRut(BufferedImage limage) {
    ...
    ImageFloat32 input = ConvertBufferedImage.convertFromSingle(limage, null, ImageFloat32.class);
  ...
  protected final double getRotationAngle(List<Contour> contours) {
    ...
    int dx = x[contours.size() - 1] - x[0];
    int dy = y[contours.size() - 1] - y[0];
  ...
  protected final List<Contour> getContoursFullImage(BufferedImage limage) {
    ...
    writeIMG(VisualizeBinaryData.renderBinary(binary, null), "thresholfullimage");
    ImageUInt8 filtered = BinaryImageOps.dilate8(binary, 2, null);
  ...
  protected final List<Contour> getContours(BufferedImage limage) {
    ...
    while (w < 200) {
      ...
      writeIMG(image, "subimage_contours");
      ImageFloat32 input = ConvertBufferedImage.convertFromSingle(image, null, ImageFloat32.class);
      ...
      threshold = GThresholdImageOps.computeOtsu(input, 0, 256);
      ThresholdImageOps.threshold(input, binary, (float) threshold, true);
      writeIMG(VisualizeBinaryData.renderBinary(binary, null), "threshold_contours");
      ImageUInt8 filtered = BinaryImageOps.dilate8(binary, 2, null);
      filtered = BinaryImageOps.erode8(filtered, 9, null);
      filtered = BinaryImageOps.dilate8(filtered, 10, null);
      BufferedImage bImage = VisualizeBinaryData.renderBinary(filtered, null);
      writeIMG(bImage, "contornos_contours");
      ...
      try {
        limage =
            ImageIO.read(new File(
                "C:\\Users\\eosorio\\Documents\\CPruebas\\pruebas\\K20150630210309843_0010.jpg"));
        ImageFloat32 input =
