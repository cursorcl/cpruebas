...
public class MenuGrabarListener implements EventHandler<ActionEvent> {
	...
	public void handle(ActionEvent event) {
		...
		for (final ItemList item : items) {
			...
			RespuestasEsperadasPrueba respuesta = new RespuestasEsperadasPrueba.Builder().anulada(false)
					.ejeTematico(item.thematic).habilidad(item.hability).mental(isMental).name(itemName).numero(item.nro)
			...
			Preguntas pregunta = new Preguntas.Builder().name(item.question).numero(item.nro).respuesta(respuesta)
					.prueba(prueba).build();
			pregunta = (Preguntas) defPrueba.save(pregunta);
	...
	private List<Alternativas> processAlteratives(ItemList item, RespuestasEsperadasPrueba respuesta) {
		...
		for (int n = 0; n < nroAlternativas; n++) {
			...
			final String texto = item.alternatives.get(n);
			final Alternativas alternative = new Alternativas.Builder().name(altName).numero(n).texto(texto)
