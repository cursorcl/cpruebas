...
public class PersistenceService implements IPersistenceService {
   ...
   * (non-Javadoc)
   * 
   * @see cpruebas.osgi.persistence.IPersistenceService#save(cl.eos.interfaces. entity.IEntity)
   */
  @Override
   ...
   * (non-Javadoc)
   * 
   * @see cpruebas.osgi.persistence.IPersistenceService#delete(cl.eos.interfaces .entity.IEntity)
   */
  @Override
  ...
  public void findById(final Class<? extends IEntity> entityClazz, final Long id,
    ...
    final Task<IEntity> task = new Task<IEntity>() {
      ...
      protected IEntity call() throws Exception {
        ...
        String strQuery =
            String.format("select c from %s c where c.id = :id", strEntity.toLowerCase());
    ...
    final Task<List> task = new Task<List>() {
      ...
      protected List call() throws Exception {
        ...
        if (ids != null) {
          ...
          String strEntity = entityClazz.getSimpleName();
          String strQuery =
              String.format("select c from %s c where c.id in (" + listaIds.toString() + ")",
  ...
  public void findByName(final Class<? extends IEntity> entityClazz, final String name,
    ...
    final Task<IEntity> task = new Task<IEntity>() {
      ...
      protected IEntity call() throws Exception {
        ...
        String strEntity = entityClazz.getSimpleName();
        Query query =
            eManager.createQuery(String
                .format("select c from %s c where c.name = :name", strEntity));
        if (query != null) {
    ...
    final Task<Pair<String, Pair<Integer, List<String>>>> task =
        new Task<Pair<String, Pair<Integer, List<String>>>>() {
          ...
          protected Pair<String, Pair<Integer, List<String>>> call() {
            ...
            Pair<String, Pair<Integer, List<String>>> pair =
            ...
            for (Object fila : filas) {
              ...
              if (query != null) {
                try {
                  ...
                  query.executeUpdate();
                  eManager.getTransaction().commit();
                  updateMessage(String.format("Procesado[%d]: %s", row++, strRegister.toString()
                      .trim()));
                } catch (Exception e) {
          ...
          public void run() {
            ...
            if (errores.isEmpty()) {
              Dialogs
                  ...
                  .masthead("")
                  .message(
                      "Ha finalizado proceso de importación de [" + pair.getSecond().getFirst()
                          + "] registros para tabla [" + pair.getFirst() + "]").showInformation();
            } else {
