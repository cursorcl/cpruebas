...
public class Nivel_ComparativoColegioEjeEvaluacionView extends AFormView implements EventHandler<ActionEvent> {
    ...
    if (entity instanceof R_Curso) {
      ...
      for (Object iEntity : list) {
        ...
        Optional<R_TipoCurso> op =
            lstTipoCursos.stream().filter(t -> t.getId().equals(curso.getTipocurso_id())).findFirst();
  ...
  private void generarReporte() {
    if (evaluacionesPrueba == null || nivelEvaluacion == null) {
      return;
    }
    ...
    Task<Void> task = new Task<Void>() {
      ...
      protected Void call() throws Exception {
        ...
        Map<String, Object> params =
            MapBuilder.<String, Object>unordered().put("nivelevaluacion_id", nivelEvaluacion.getId()).build();
        ...
        for (R_EvaluacionPrueba eval : evaluacionesPrueba) {
          ...
          params = MapBuilder.<String, Object>unordered().put("prueba_id", eval.getPrueba_id()).build();
          ...
          for (R_PruebaRendida pruebaRendida : pruebasRendidas) {
            ...
            R_Curso curso = cursoList.stream().filter(c -> c.getId().equals(alumno.getCurso_id())).findFirst().orElse(null);
            if (curso == null) // No hay curso asociado al alumno, algo imposible.
              continue;
            R_TipoCurso tipoCurso = tipoCursoList.stream().filter(t -> t.getId().equals(curso.getTipocurso_id())).findFirst().orElse(null);
            if (tipoCurso == null)
              continue;
        ...
        updateMessage("Generando la tabla de ejes temáticos");
