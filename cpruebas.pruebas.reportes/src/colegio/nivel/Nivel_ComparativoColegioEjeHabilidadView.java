...
public class Nivel_ComparativoColegioEjeHabilidadView extends AFormView implements EventHandler<ActionEvent> {
  ...
  public void onDataArrived(List<Object> list) {
    if (list != null && !list.isEmpty()) {
      ...
      if (entity instanceof R_Curso) {
        ...
        Long[] ids =
            cursoList.parallelStream().map(c -> c.getTipocurso_id()).distinct().toArray(size -> new Long[size]);
        lstTipoCurso = FXCollections.observableArrayList(controller.findByAllIdSynchro(R_TipoCurso.class, ids));
      }
  ...
  private void generarReporte() {
    if (evaluacionesPrueba == null || evalEjeTematicoList == null  || lstTipoCurso == null) {
      return;
    }
    long tipoAlumno = cmbTipoAlumno.getSelectionModel().getSelectedItem().getId();

    llenarColumnas(lstTipoCurso);
    int nroTiposCursos = lstTipoCurso.size();
    ...
    for (R_EvaluacionPrueba eval : evaluacionesPrueba) {
      ...
      // contenido
      R_TipoCurso tipoCurso = lstTipoCurso.stream().filter(t -> t.getId().equals(eval.getTipocurso_id())).findFirst().orElse(null);
      if (tipoCurso == null)
        continue;
      int index = lstTipoCurso.indexOf(tipoCurso);
      if (index == -1) {
        continue;
      }
      ...
      for (R_PruebaRendida pruebaRendida : pruebasRendidas) {
        ...
        Optional<R_Alumno> opAlumno =
            alumnos.stream().filter(a -> pruebaRendida.getAlumno_id().equals(a.getId())).findFirst();
   ...
   * @return Par <Preguntas buenas, Total de Preguntas> del eje.
   */
  private Pair<Integer, Integer> obtenerBuenasTotalesEje(String respuestas,
