...
public class Nivel_ComparativoColegioEjeHabilidadxCursoView extends AFormView implements EventHandler<ActionEvent> {
  ...
  private List<R_TipoCurso> listaTipoCursos;
  ...
  public void onDataArrived(List<Object> list) {
    if (list != null && !list.isEmpty()) {
      ...
      if (entity instanceof R_EvaluacionPrueba) {
        ...
        Long[] ids = listaEvaluacionesPrueba.parallelStream().map(e -> e.getTipocurso_id()).distinct()
            .toArray(size -> new Long[size]);

        listaTipoCursos = controller.findByAllIdSynchro(R_TipoCurso.class, ids);
  ...
  private void makeTable() {
    ...
    for (R_EvaluacionPrueba evPrueba : listaEvaluacionesPrueba) {
      Optional<R_TipoCurso> op =
          listaTipoCursos.stream().filter(t -> t.getId().equals(evPrueba.getTipocurso_id())).findFirst();
      if (!op.isPresent())
        continue;
  ...
  private List<OTUnCursoUnEjeHabilidad> evaluarUnAlumno(R_PruebaRendida pRendida,
      List<R_RespuestasEsperadasPrueba> respEsperadas, int nAlumno, List<OTUnCursoUnEjeHabilidad> listaOTUnCurso,
      List<R_PruebaRendida> lstPRendidas) {
  ...
  private Map<IEntity, List<OTTipoCursoRangos>> buildDataReport(int nroTipoCurso,
      List<R_Ejetematico> listaEjesTematicos, List<R_Habilidad> listaHablidades) {
    ...
    for (R_EvaluacionPrueba evaluacion : listaEvaluacionesPrueba) {
      ...
      if (rEsperadas == null) {
        Map<String, Object> parameters =
            MapBuilder.<String, Object>unordered().put("prueba_id", evaluacion.getPrueba_id()).build();
      ...
      for (R_RespuestasEsperadasPrueba rEsperada : rEsperadas) {
        ...
        if (!report.containsKey(hab)) {
          List<OTTipoCursoRangos> list =
              Stream.generate(OTTipoCursoRangos::new).limit(nroTipoCurso).collect(Collectors.toList());
          report.put(hab, list);
        }
  ...
  private void generarReporte() {
    ...
    for (R_EvaluacionPrueba evaluacion : listaEvaluacionesPrueba) {
      R_TipoCurso tipoCurso = listaTipoCursos.stream().filter(t -> t.getId().equals(evaluacion.getTipocurso_id()))
    ...
    Task<Void> task = new Task<Void>() {
      ...
      protected Void call() throws Exception {
        ...
        final List<R_Ejetematico> listaEjesTematicos = new ArrayList<>();
        final List<R_Habilidad> listaHablidades = new ArrayList<>();
        final int nroTipoCurso = lstTipoCurso.size();
        final Map<IEntity, List<OTTipoCursoRangos>> reporte = buildDataReport(nroTipoCurso, listaEjesTematicos, listaHablidades);

        updateMessage("Generando reporte");
        // Obtener la cantidad de niveles que existen.
        int n = 0;
        int total = listaEvaluacionesPrueba.size();
        ...
        for (R_EvaluacionPrueba evaluacion : listaEvaluacionesPrueba) {
          ...
          updateMessage("Procesando evaluación :" + ++n);
          updateProgress(n, total);
          
          Optional<R_TipoCurso> op =
              listaTipoCursos.stream().filter(t -> t.getId().equals(evaluacion.getTipocurso_id())).findFirst();
          if (!op.isPresent())
            continue;
          R_TipoCurso tipoCurso = op.get();
          ...
          for (R_PruebaRendida pRendida : lstPRendidas) {
            if (tipoAlumno != Constants.PIE_ALL && pRendida.getTipoalumno_id() != null
                && tipoAlumno != pRendida.getTipoalumno_id()) {
              // En este caso, no se considera este alumno para el
              // cálculo.
              continue;
            }
            R_Alumno alumno = controller.findByIdSynchro(R_Alumno.class, pRendida.getAlumno_id());
            updateMessage("Procesando alumno:" + alumno.toString());
  ...
  /**
   * Obtiene las respuestas esperadas de una prueba. Luego la regustra en un mapa asociando la
   * prueba con sus respuestas, logrando así reutilizar las búsquedas.
   * 
   * @param prueba_id Identificador de la prueba
