...
public class Nivel_ComparativoColegioHabilidadesView extends AFormView implements EventHandler<ActionEvent> {
  ...
  private void handleColegios() {
    ...
    if (colegio != null) {
      ...
      Map<String, Object> param = MapBuilder.<String, Object>unordered().put(COLEGIO_ID, colegio.getId()).build();
      ...
      if (entity instanceof R_Curso) {
        ...
        for (Object iEntity : list) {
          ...
          Optional<R_TipoCurso> op =
              lstTipoCursos.stream().filter(c -> c.getId().equals(curso.getTipocurso_id())).findFirst();
          if (!op.isPresent())
            continue;
  ...
  private void llenarColumnas(List<R_TipoCurso> pCursoList, ObservableList<R_RangoEvaluacion> rangos) {
    Runnable r = () -> {
  ...
  private void generarReporte() {
    ...
    Task<Void> task = new Task<Void>() {
      ...
      protected Void call() throws Exception {
        ...
        int nroTipoCursos = tipoCursoList.size();
        ...
        for (R_EvaluacionPrueba eval : evaluacionesPrueba) {
          updateMessage("Procesando evaluación prueba:" + (++avance));
          ...
          List<R_PruebaRendida> pruebasRendidas = controller.findByParamsSynchro(R_PruebaRendida.class, parameters);
          List<R_RespuestasEsperadasPrueba> respEsperadas = mapRespEsperadas.get(eval.getPrueba_id());
          if (respEsperadas == null) {
            parameters = MapBuilder.<String, Object>unordered().put("prueba_id", eval.getPrueba_id()).build();
          ...
          for (R_PruebaRendida pruebaRendida : pruebasRendidas) {
            ...
            R_Curso curso =
                cursoList.stream().filter(c -> c.getId().equals(alumno.getCurso_id())).findFirst().orElse(null);
            if (curso == null) // No hay curso asociado al alumno, algo imposible.
              continue;
            R_TipoCurso tipoCurso =
                tipoCursoList.stream().filter(t -> t.getId().equals(curso.getTipocurso_id())).findFirst().orElse(null);
            if (tipoCurso == null)
              continue;
            ...
            // columnas
            // asociadas.
            int index = tipoCursoList.indexOf(tipoCurso);
        ...
        updateMessage("Generando la tabla de habilidades");
  ...
  private float obtenerPorcentaje(String respuestas, List<R_RespuestasEsperadasPrueba> respEsperadas,
