...
public class PorObjetivosView extends AFormView {
  private static Logger log = Logger.getLogger(PorObjetivosView.class.getName());
  @FXML
      ...
      if (pRendidas != null && !pRendidas.isEmpty() && respEsperadas != null && !respEsperadas.isEmpty()) {
        ...
        Task<Void> task = new Task<Void>() {
          ...
          protected Void call() throws Exception {
            ...
            updateMessage("Preprocesando los datos");


            final List<XItemTablaObjetivo> reporte =
                XUtilReportBuilder.reporteCurso(lstEvaluacion, pRendidas, respEsperadas, tipoAlumno);
            ...
            for (int n = 0; n < items.size(); n++) {
              updateMessage("Procesando:" + items.get(n).getObjetivo().getName());
              updateProgress((n + 1), total);
              final int idx = n;
  ...
  public void onFound(IEntity entity) {
    if (entity instanceof R_EvaluacionPrueba) {
      ...
      curso = PersistenceServiceFactory.getPersistenceService().findByIdSynchro(R_Curso.class,
          evaluacionPrueba.getCurso_id());
