...
//                .add(eval.stream().filter(e -> e.objetivo.equals(o.getObjetivo())).findFirst().orElse(null)));
    ...
    public static Pair<List<Curso>, List<ItemTablaObjetivo>> buildReportColegio(List<PruebaRendida> pruebas) {
        ...
        List<Curso> cursosOrdenados = eval.keySet().stream().sorted(Comparadores.odenarCurso())
        ...
        int n = 0;
        for (Curso key : cursosOrdenados) {
            List<ItemObjetivo> values = eval.get(key);
    ...
    public static Pair<List<TipoCurso>, List<ItemTablaObjetivo>> buildReportColegioxNivel(List<PruebaRendida> pruebas) {
        ...
        List<TipoCurso> tipoCursosOrdenados = eval.keySet().stream().sorted(Comparadores.comparaTipoCurso())
                .collect(Collectors.toList());
        
        final int size = tipoCursosOrdenados.size();
        objetivos.forEach(o -> o.setItems(Stream.generate(ItemObjetivo::new).limit(size).collect(Collectors.toList())));
        int n = 0;
        for (TipoCurso key : tipoCursosOrdenados) {
            List<ItemObjetivo> values = eval.get(key);
            objetivos.forEach(o -> o.getItems().set(n,
                    values.stream().filter(e -> e.objetivo.equals(o.getObjetivo())).findFirst().orElse(null)));
        }
...
//    public static Pair<List<Colegio>, Pair< List<Curso>, List<ItemTablaObjetivo>>> buildReportComunal(List<PruebaRendida> pruebas) {
//        Optional<PruebaRendida> first = pruebas.stream().findFirst();
//        if (!first.isPresent())
//            return null;
//        List<RespuestasEsperadasPrueba> respEsperadas = first.get().getEvaluacionPrueba().getPrueba().getRespuestas();
...
//    }
//
//    public static List<ItemTablaObjetivo> buildReportComunalxCurso(List<PruebaRendida> pruebas) {
//        IResultado evaluacion = evaluador.evaluar(pruebas);
//        List<RespuestasEsperadasPrueba> respEsperadas = pruebas.get(0).getEvaluacionPrueba().getPrueba()
//                .getRespuestas();
//        List<ItemTablaObjetivo> objetivos = buildFixedReportClumns(respEsperadas);
...
//    }
//
//    public static List<ItemTablaObjetivo> buildReportComunalxNivel(List<PruebaRendida> pruebas) {
//        IResultado evaluacion = evaluador.evaluar(pruebas);
//        List<RespuestasEsperadasPrueba> respEsperadas = pruebas.get(0).getEvaluacionPrueba().getPrueba()
//                .getRespuestas();
//        List<ItemTablaObjetivo> objetivos = buildFixedReportClumns(respEsperadas);
