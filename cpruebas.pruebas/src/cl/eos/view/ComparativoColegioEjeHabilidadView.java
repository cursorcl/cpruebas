...
public class ComparativoColegioEjeHabilidadView extends AFormView implements
   ...
   * 3) Se obtienen los porcentajes de aprobacion de de cada curso con respecto a cada eje y
   * habilidad.
   */
  private void generarReporte() {
    ...
    Map<EjeTematico, List<OTPreguntasEjes>> mapEjes =
        new HashMap<EjeTematico, List<OTPreguntasEjes>>();
    ...
    for (EvaluacionPrueba eval : evaluacionesPrueba) {
      ...
      for (PruebaRendida pruebaRendida : pruebasRendidas) {
        ...
        // Obtengo el index de la columna que tengo que llenar (mas 1 por que la primera es de
        // contenido
        int index = cursoList.indexOf(pruebaRendida.getAlumno().getCurso());

        totalAlumnos[index] = pruebaRendida.getAlumno().getCurso().getAlumnos().size();
  ...
  private void generarTablaEvaluaciones(
      Map<EvaluacionEjeTematico, List<OTPreguntasEvaluacion>> mapEvaluaciones, int[] totalAlumnos,
