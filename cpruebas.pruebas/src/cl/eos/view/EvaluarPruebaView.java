...
public class EvaluarPruebaView extends AFormView {
  private class EHandlerCmbCurso implements EventHandler<ActionEvent> {
    ...
    public void handle(ActionEvent event) {
      ...
      if (curso != null) {
        ...
        params = MapBuilder.<String, Object>unordered().put("curso_id", curso.getId())
            .put("colegio_id", colegio.getId()).put("prueba_id", prueba.getId()).build();
        final List<R_EvaluacionPrueba> listEvaluaciones =
            controller.findByParamsSynchro(R_EvaluacionPrueba.class, params);
        if (listEvaluaciones != null && !listEvaluaciones.isEmpty()) {
          ...
          List<R_Alumno> aNoRendido =
              lstAlumnos.stream().filter(a -> !pRendAlumno.contains(a.getId())).collect(Collectors.toList());
          for (R_PruebaRendida r : lstPRendidas) {
            R_Alumno oAlumno =
                lstAlumnos.stream().filter(a -> a.getId().equals(r.getAlumno_id())).findFirst().orElse(null);
            OTPruebaRendida ot = new OTPruebaRendida(r, oAlumno);
            R_TipoAlumno tipoAlumno =
                lstTipoAlumno.stream().filter(t -> t.getId().equals(r.getTipoalumno_id())).findFirst().orElse(null);
            ot.setTipoAlumno(tipoAlumno);
            R_RangoEvaluacion eval =
                lstRangos.stream().filter(t -> t.getId().equals(r.getRango_id())).findFirst().orElse(null);
            ot.setNivel(eval);
            oList.add(ot);
          }
          if (aNoRendido != null && !aNoRendido.isEmpty()) {
            for (final R_Alumno alumno : aNoRendido) {
              ...
              R_TipoAlumno tipoAlumno = lstTipoAlumno.stream().filter(t -> t.getId().equals(alumno.getTipoalumno_id()))
  ...
  @FXML
  private ComboBox<R_Curso> cmbCursos;
  @FXML
  private ComboBox<R_Profesor> cmbProfesor;
  @FXML
  ...
  private void definirTablaListadoPruebas() {
    ...
    nombresCol.setCellValueFactory(new PropertyValueFactory<OTPruebaRendida, String>("nombre"));
  ...
  protected void handlerGrabar() {
    ...
    if (validate()) {
      ...
      if (!lstEvaluaciones.contains(evalPrueba)) {
        ...
        evalPrueba = (R_EvaluacionPrueba)save(evalPrueba);
  ...
  protected void handlerLeerImagenes() throws IOException {
    ...
    final FileChooser.ExtensionFilter imageExtFilter =
    ...
    if (files != null && !files.isEmpty()) {
      ...
      final Task<Pair<ObservableList<R_Alumno>, ObservableList<R_PruebaRendida>>> task =
          new Task<Pair<ObservableList<R_Alumno>, ObservableList<R_PruebaRendida>>>() {
            @Override
            protected Pair<ObservableList<R_Alumno>, ObservableList<R_PruebaRendida>> call() throws Exception {
              ...
              int n = 1;
              final Pair<ObservableList<R_Alumno>, ObservableList<R_PruebaRendida>> results =
                  new Pair<ObservableList<R_Alumno>, ObservableList<R_PruebaRendida>>();
      ...
      task.setOnSucceeded(event -> {
        ...
        if (pruebas != null && !pruebas.isEmpty()) {
          ...
          for (final R_PruebaRendida pr : pruebas) {
            ...
            Optional<R_RangoEvaluacion> oRango =
                rangos.stream().filter(rr -> rr.getId().equals(pr.getRango_id())).findFirst();
        ...
        final Runnable r = () -> {
          ...
          info.setContentText(
              String.format("Se han procesado %d archivos exitosos de un total de %d.", nPruebas, nPruebas + nMalas));
          info.show();
        };
        Platform.runLater(r);
      });
  ...
  public void onFound(IEntity entity) {
    ...
    if (entity instanceof R_Prueba) {
      ...
      params = MapBuilder.<String, Object>unordered().put("prueba_id", prueba.getId())
          .put("curso_id", prueba.getCurso_id()).build();
