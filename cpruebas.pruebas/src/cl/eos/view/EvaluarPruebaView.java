...
public class EvaluarPruebaView extends AFormView {
		...
		public void handle(ActionEvent event) {
			...
			if (curso != null) {
				...
				if (evalPrueba == null) {
					...
					for (PruebaRendida pRendida : evalPrueba
							.getPruebasRendidas()) {
						oList.add(new OTPruebaRendida(pRendida));
					}
					tblListadoPruebas.setItems(oList);
	...
	protected void handlerLeerImagenes() throws IOException {
		...
		FileChooser.ExtensionFilter pngExtFilter = new FileChooser.ExtensionFilter(
				"PNG files (*.png)", "*.png");
		fileChooser.getExtensionFilters().add(pngExtFilter);
		FileChooser.ExtensionFilter jpgExtFilter = new FileChooser.ExtensionFilter(
				"JPG files (*.jpg)", "*.jpg");
		fileChooser.getExtensionFilters().add(jpgExtFilter);
		fileChooser.setTitle("Seleccione Imégenes Respuesta");
		List<File> files = fileChooser.showOpenMultipleDialog(null);

		// final TareaProcesaEvaluacionScanner procesador =
		// new TareaProcesaEvaluacionScanner(prueba, cmbCursos.getValue(),
		// files);
		//
		...
		// @Override
		// public void handle(WorkerStateEvent event) {
		// System.out.println("setOnFailed " + event.toString());
		// // Algo falló por lo tanto hay que avisar.
		// Dialogs.create().owner(null).title("Importación no completada.")
		...
		// .message("La operación ha sido cancelada por el usuario.").showInformation();
		//
		// }
		// });
		// //Dialogs.create().owner(null).title("Procesando respuestas.").showWorkerProgress(procesador);
		// service.execute(procesador);

		ExtractorResultadosPruebas procesador = ExtractorResultadosPruebas
				.getInstance();
		tblListadoPruebas.getItems().clear();
		for (File archivo : files) {
			OTResultadoScanner resultado = procesador.process(archivo,
					prueba.getNroPreguntas());
			PruebaRendida pRendida = obtenerPruebaRendida(resultado);
			evalPrueba.getPruebasRendidas().add(pRendida);
