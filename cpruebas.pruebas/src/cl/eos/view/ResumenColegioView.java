...
public class ResumenColegioView extends AFormView implements
    ...
    for (Object evaluacionPrueba : list) {
      ...
      resumenCurso.setAlumnosEvaluados(Utils.redondeo2Decimales((((float) totalEvaluados / (float) totalAlumnos) * 100f)));
      resumenCurso.setAlumnosAprobados(Utils.redondeo2Decimales((((float) totalAprobados / (float) totalEvaluados) * 100f)));
      resumenCurso.setAlumnosReprobados(Utils.redondeo2Decimales((((float) totalReprobados / (float) totalEvaluados) * 100f)));
	...
	private void generarDatosResumenGeneral() {
		...
		for (Entry<Integer, EvaluacionEjeTematico> ttEvaluacion : tituloEvaluacion
			...
			if (evaluacion != null) {
				...
				float valor = ((float) evaluacion / (float) resumenTotal
						.getTotalEvaluados()) * 100f;
