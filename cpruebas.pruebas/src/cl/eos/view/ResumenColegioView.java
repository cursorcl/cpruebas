...
public class ResumenColegioView extends AFormView implements EventHandler<ActionEvent> {
  ...
  private void generarReporteCursos(List<Object> list) {
    ...
    for (Object evaluacionPrueba : list) {
      ...
      resumenCurso.setAlumnosEvaluados(((float) totalEvaluados / (float) totalAlumnos) * 100f);
      resumenCurso.setAlumnosAprobados(((float) totalAprobados / (float) totalEvaluados) * 100f);
      resumenCurso.setAlumnosReprobados(((float) totalReprobados / (float) totalEvaluados) * 100f);
