...
public class ResumenXAlumnoEjeHabilidadView extends AFormView implements EventHandler<ActionEvent> {
    ...
    if (entity instanceof EvaluacionPrueba) {
      ...
      if (pRendidas != null && !pRendidas.isEmpty()) {
        List<RespuestasEsperadasPrueba> respEsperadas =
            evaluacionPrueba.getPrueba().getRespuestas();
   ...
   */
  private List<OTAlumnoResumen> obtenerPuntos(EvaluacionPrueba evaluacionPrueba,
      List<EjeTematico> lstEjes, List<Habilidad> lstHabs) {
   ...
   * @param ahb La Habilidad en base al que se realiza el calculo.
  ...
  private TableColumn getFixedColumns(String name, final int index) {
  ...
  private void poblarGraficos(int index) {
    ...
    int idx = 0;
    for (EjeTematico eje : lstOtEjes) {
      seriesE.getData()
          .add(new XYChart.Data<String, Number>(String.valueOf(idx + 1), porcentajes.get(idx++)));
    }
    ...
    for (Habilidad hab : lstOtHabs) {
      seriesH.getData()
          .add(new XYChart.Data<String, Number>(String.valueOf(idx + 1), porcentajes.get(idx++)));
    }
    grfHabilidades.setLegendSide(Side.RIGHT);
